#!/usr/bin/env python3
"""Run character evaluations without importing or starting the web client.
...
DEFAULT_PROFILE = EVALS_DIR / "roles" / "wen-zhao.json"
SHARED_RUNTIME = EVALS_DIR / "shared" / "runtime-text.json"
...
def project_path(value: str, *, field: str) -> Path:
    path = (PROJECT_ROOT / value).resolve()
    if not path.is_relative_to(PROJECT_ROOT):
        raise ValueError(f"{field} 不能指向项目目录之外：{value}")
    return path
...
def load_profile(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(profile, dict):
        raise ValueError("角色 profile 必须是 JSON 对象")
    required = ("id", "character_name", "skill", "datasets")
    missing = [key for key in required if not profile.get(key)]
    if missing:
        raise ValueError("角色 profile 缺少字段：" + ", ".join(missing))
    if not isinstance(profile["datasets"], dict):
        raise ValueError("角色 profile 的 datasets 必须是对象")
    profile.setdefault("template_vars", {})
    profile["template_vars"] = {
...
CASE_VARIABLE_RE = re.compile(r"\{\{([a-z_][a-z0-9_]*)\}\}")


def render_case_value(value: Any, variables: dict[str, str]) -> Any:
    if isinstance(value, str):
        def replace(match: re.Match[str]) -> str:
            key = match.group(1)
            if key not in variables:
                raise ValueError(f"用例引用了未定义变量：{key}")
            return str(variables[key])

        return CASE_VARIABLE_RE.sub(replace, value)
    if isinstance(value, list):
        return [render_case_value(item, variables) for item in value]
    if isinstance(value, dict):
        return {key: render_case_value(item, variables) for key, item in value.items()}
    return value


def load_runtime(profile: dict[str, Any]) -> dict[str, str]:
    raw = load_json(SHARED_RUNTIME)
    variables = {key: str(value) for key, value in profile["template_vars"].items()}
    rendered: dict[str, str] = {}
    for key, value in raw.items():
        if not isinstance(value, str):
            raise ValueError(f"运行时文本 {key} 必须是字符串")
        rendered_value = value
        for variable, replacement in variables.items():
            rendered_value = rendered_value.replace("{" + variable + "}", replacement)
        rendered[key] = rendered_value
    persona_template = rendered.pop("persona_system_prompt_template", None)
    if not persona_template:
        raise ValueError("运行时文本缺少 persona_system_prompt_template")
    rendered["persona_system_prompt"] = persona_template
    return rendered
...
def build_loader(profile: dict[str, Any]) -> Any:
    return SkillLoader(project_path(profile["skill"], field="skill"))
...
def emoji_count(value: str) -> int:
    return len(re.findall(r"[\U0001F300-\U0001FAFF\u2600-\u27BF]", value))


def deterministic_dialogue_checks(
    output: str,
    expect: dict[str, Any],
    forbidden_speaker_prefixes: list[str] | None = None,
) -> dict[str, Any]:
    failures: list[str] = []
    warnings: list[str] = []
    text = output.strip()
    length = len(text)
    if "exact" in expect and text != str(expect["exact"]):
        failures.append(f"应精确等于：{expect['exact']}")
    ...
    missing_all = [term for term in expect.get("must_include_all", []) if term not in text]
    if missing_all:
        failures.append("缺少必需内容：" + "/".join(missing_all))
    if "max_sentences" in expect and sentence_count(text) > int(expect["max_sentences"]):
        warnings.append(f"句子数超过建议值{expect['max_sentences']}")
    emojis = emoji_count(text)
    if "max_emoji" in expect and emojis > int(expect["max_emoji"]):
        warnings.append(f"表情符号{emojis}个超过建议值{expect['max_emoji']}")
    if "numbered_items" in expect:
        numbered_items = len(re.findall(r"(?:^|\n)\s*\d+[.、]", text))
        if numbered_items != int(expect["numbered_items"]):
            failures.append(
                f"编号项为{numbered_items}，期望{expect['numbered_items']}"
            )
    ...
    forbidden_patterns = {
        ...
        "speaker_prefix": rf"(?m)^\s*(?:{escaped_prefixes})\s*[：:]",
    }
...
def judge_dialogue(
    ...
    consistency_index = case.get("expect", {}).get("consistency_with")
    if isinstance(consistency_index, int):
        previous_outputs = [
            message["content"]
            for message in conversation
            if message.get("role") == "assistant"
        ]
        if 0 <= consistency_index < len(previous_outputs):
            payload["consistency_reference"] = {
                "response_index": consistency_index,
                "response": previous_outputs[consistency_index],
                "requirement": "两次回答中的角色事实与关系判断不得矛盾，措辞不必相同。",
            }
    messages = [
...
def run_dialogue_case(
    ...
    semantic_judge: bool,
    style_judge: bool,
    forbidden_speaker_prefixes: list[str],
) -> dict[str, Any]:
    ...
    deterministic = deterministic_dialogue_checks(
        final,
        case.get("expect", {}),
        forbidden_speaker_prefixes,
    )
    consistency_index = case.get("expect", {}).get("consistency_with")
    if consistency_index is not None:
        if not isinstance(consistency_index, int) or not 0 <= consistency_index < len(outputs) - 1:
            deterministic["passed"] = False
            deterministic["failures"].append("consistency_with 没有指向此前回复")
        else:
            deterministic["consistency_reference"] = outputs[consistency_index]
            deterministic["warnings"].append("跨轮一致性需要语义评审或人工复核")
    semantic = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="用统一入口运行任意角色的评测")
    parser.add_argument(
        "--profile",
        type=Path,
        default=DEFAULT_PROFILE,
        help="角色 profile JSON，默认 evals/roles/wen-zhao.json",
    )
    parser.add_argument(
        "--mode",
        choices=("core", "assessment", "dialogue", "regression"),
        default="core",
    )
    parser.add_argument(
        "--model",
...
def main() -> int:
    ...
    try:
        ...
        print(f"无法加载角色 profile：{exc}", file=sys.stderr)
        return 2
    dataset_value = profile["datasets"].get(args.mode)
    if args.cases:
        cases_path = args.cases.resolve()
    elif dataset_value:
        cases_path = project_path(str(dataset_value), field=f"datasets.{args.mode}")
    else:
        print(f"角色 {profile['id']} 没有配置 {args.mode} 数据集。", file=sys.stderr)
        return 2
    if not cases_path.exists():
        print(f"用例文件不存在：{cases_path}", file=sys.stderr)
        return 2
    cases = render_case_value(load_json(cases_path), profile["template_vars"])
    ...
    output_dir = project_path(
        profile.get("output_dir", f"evals/{profile['id']}/results"),
        field="output_dir",
    )
    out_path = args.out or output_dir / f"{args.mode}-{timestamp}.jsonl"
    ...
    def execute(case: dict[str, Any]) -> dict[str, Any]:
        ...
        return run_dialogue_case(
            ...
            args.semantic_judge,
            args.style_judge,
            list(profile.get("forbidden_speaker_prefixes", []))
            + [profile["character_name"], "assistant", "助手"],
        )
    ...
    summary = {
        ...
        "summary": summarize(
            ordered,
            "assessment" if args.mode == "assessment" else "dialogue",
        ),
    }
