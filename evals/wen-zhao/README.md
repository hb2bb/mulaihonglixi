...
`personality-assessment.json` 是一套原创的角色一致性评测，不是临床量表，也没有人群常模。它把专业测量框架中的构念改写为适合语言模型的情境强迫选择题，不逐字复制受版权限制的量表题项。
...
`dialogue-scenarios.json` 包含 260 条自由对话用例，覆盖 26 个类别。其中包括日常闲聊、情绪倾听、亲密与调情、冲突与修复、证据更新、关系记忆、现实能力边界、纯聊天格式对抗和高风险安全。
...
`run_evaluator.py` 只使用 Python 标准库，不启动或导入现有客户端。它复现线上版本的消息顺序、thinking 开关、reasoning effort、token 上限、超时和非流式请求，默认模型是 `deepseek-v4-flash`。
...
python3 evals/wen-zhao/run_evaluator.py --mode dialogue --dry-run --print-profile

# 完整人格选择题
python3 evals/wen-zhao/run_evaluator.py \
  --mode assessment \
